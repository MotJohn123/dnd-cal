...
// POST /api/sessions/[id]/unconfirm - Unconfirm attendance
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { message: 'Attendance cancelled', session: sessionDoc },
      { status: 200 }
    );
  } catch (error: any) {
    console.error('Unconfirm attendance error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to cancel attendance' },
      { status: 500 }
    );
  }
}
